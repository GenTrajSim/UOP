...
class Data_Pre:
  def __init__(self, total_std_path, total_var_path, maxatom=50,  d_cut = 5,  len_dictionary = 5,  Tranining = False):
  ...
  def read_npy_file(self, filename1, filename2, filename_list):
    filename1 = str(filename1)
    filename2 = str(filename2)
    if self.Tranining == True:
      for i in range(len(filename_list)):
        filename_list[i] = str(filename_list[i])
