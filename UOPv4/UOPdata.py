...
class create_masks:
    def __init__(self, noise_C,token_noise=0.2, iterT = 50, training=True,dictionary = {'MASK':0, 'C':1, 'O':2, 'N':3, 'H':4, 'CLAS':5, 'TEMP':6, 'PRESS':7}):
        ...
        self.alpha_bars_tf = tf.constant(alpha_bars, dtype=tf.float32)
    def Create_noise(self, tokens_c, coords_c):
        ...
        weight_token = tf.cast((tf.random.uniform(shape=tf.shape(NO_padding)))*(len(self.dictionary)-4)+1, dtype=tf.int32)
        ...
        alpha_bar_t = tf.gather(self.alpha_bars_tf, t_indices)  # shape=(batch_size,)
...
class DiffusionSampler:
    ...
    def sample(self, token, press, temp,  x_t=None):
        if x_t is None:
        ...
        for t in reversed(range(1, self.iterT + 1)):
            ...
            out = self.score_model(token, x_t, t_tensor, press, temp, training=False)
            (pred_tokens, _, pred_temp, pred_press, pred_noise, _, _) = out
            pred_tokens = tf.argmax(pred_tokens, axis=-1) 
            token = pred_tokens
            press = press #pred_press
            temp = temp #pred_temp
