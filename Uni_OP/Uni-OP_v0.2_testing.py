...
path_coord =  glob.glob('./Data_111/displacement/coord/5.333.*.npy')
tf.print("==========path_coord===========")
#tf.print(path_coord)
#glob.glob('../SAVE/testing/2/coord/*.npy')
#glob.glob('../SAVE/testing/1/coord/*.npy') + glob.glob('../SAVE/testing/2/coord/*.npy')
#glob.glob('./Data_111/displacement3/coord/5.*.npy') + glob.glob('./Data_111/displacement3/coord/6.*.npy')
#glob.glob('./Data_111/ice1h/coord/*.npy') + glob.glob('./Data_111/ice1c/coord/*.npy') + glob.glob('./Data_111/liq/coord/*.npy')
# glob.glob('./Data_111/displacement/coord/*.npy') + glob.glob('./Data_111/displacement2/coord/*.npy') + glob.glob('./Data_111/displacement3/coord/*.npy') + glob.glob('./Data_111/liq/coord/*.npy') + glob.glob('./Data_111/ice1h/coord/*.npy') + glob.glob('./Data_111/ice1c/coord/*.npy') + glob.glob('./Data_111/displacement3/coord/*.npy') #+ glob.glob('./Data_111/liq/coord/*.npy')
...
#path_test = glob.glob('../SAVE/testing/3/coord/*.npy')
#path_test = glob.glob('../SAVE/testing/crystal/20.ice12/2/coord/*.npy')
tf.print(cal_pathtest)
path_test = glob.glob(cal_pathtest) # glob.glob('../SAVE/MultiPT/P5000_T200/coord/*.npy')
tf.print(path_test)
#glob.glob('./Test_Dataset/ice1h/coord/*.npy') + glob.glob('./Test_Dataset/ice1c/coord/*.npy') + glob.glob('./Data_111/liq/coord/*.npy')
...
class Data_Feeder:
    ...
    def read_npy_file(self, filename, filename2):
        ...
        #label = int(filename.split('d/')[1].split('.')[0]) #label
        atom_N = data.shape[0]
        #indices = self.indices_distance(data2,atom_N,self.d_cut) # <d_cut index indices
        indices = self.indices_distance_d(data2,atom_N,8,45)################## !!!!!!!!!!!!!!!!!!50
        LOCLA_token = []
        LOCLA_label = []
        LOCLA_atom_edge = []
        LOCLA_coord = []
...
def create_masks_date(token, coord, dist, label, 
    ...
    mask_noise = mask_coord*crystal_mask_coord*5 #normal 4
    #print (mask_coord)
...
def loss_function_1(pred_token, orign_token, 
    ...
    #crystal_mask_coord2= tf.cast(tf.where(tf.not_equal(real_crystal,0),0,1),tf.float32) * 0.1
    #crystal_mask_coord = tf.tile(tf.expand_dims(crystal_mask_coord,-1),multiples=[1,131])  ###check
    #crystal_mask_coord = tf.tile(tf.expand_dims(crystal_mask_coord,-1),multiples=[1,1,3])  ###check
    #crystal_mask_coord2= tf.tile(tf.expand_dims(crystal_mask_coord2,-1),multiples=[1,131])  ###check
    #crystal_mask_coord2= tf.tile(tf.expand_dims(crystal_mask_coord2,-1),multiples=[1,1,3])  ###check
    ...
    #crystal_mask_dist2 = tf.cast(tf.where(tf.not_equal(real_crystal,0),0,1),tf.float32) * 0.1
    ...
    if new_dist is not None:
        dist_MASK = tf.tile(tf.expand_dims(mask_token, axis=-1),
                            multiples=[1,1,tf.shape(mask_token)[-1]])
...
checkpoint_path = "../SAVE/1.0_DAMN_liq20/checkpoint_COORD_1.00_vDAMN_ln_liq20/train"
                 #"../SAVE/1.0_DAMN_liq16/checkpoint_COORD_1.00_vDAMN_ln_liq16_1/train"
...
def train_step(orign_token, label, edge, coord, dist, len_dictionary):
    noise_size_set = random.uniform(0.3,0.5)
    #noise_size_set = 0.6
    (
        mask_token, 
        mask_edge,
        ...
        mask_prob = random.uniform(0.5,0.6), #1 ,
    ...
    #pathname = '../SAVE/1.0_DAMN_liq20/MultiPT/P5000_T200/'
    pathname = '../SAVE/1.0_DAMN_liq20/MultiPT/'+cal_filename+'/'
    filename_label_o = '.o.train.label.log'
    filename_label_o = pathname + str(epoch + 1) + filename_label_o
    flo = open(filename_label_o,'a')
    filename_label_p = '.p.train.label.log'
    filename_label_p = pathname + str(epoch + 1) + filename_label_p
    flp = open(filename_label_p,'a')
    tf.print( tf.expand_dims(label,-1), summarize=500000 , output_stream = 'file://'+flo.name )
    tf.print( tf.expand_dims( tf.math.argmax(pred_crystal, axis=-1) ,-1), summarize=500000 , output_stream = 'file://'+flp.name )
    filename_coord_o = '.o.train.coord.log'
    filename_coord_o = pathname + str(epoch + 1) + filename_coord_o
    fco = open(filename_coord_o,'a')
    filename_coord_m = '.m.train.coord.log'
    filename_token_m = '.m.train.token.log'
    filename_coord_m = pathname + str(epoch + 1) + filename_coord_m
    fcm = open(filename_coord_m,'a')
    filename_coord_p = '.p.train.coord.log'
    filename_coord_p = pathname + str(epoch + 1) + filename_coord_p
    fcp = open(filename_coord_p,'a')
    tf.print( coord ,summarize=500000, output_stream = 'file://'+fco.name)
    tf.print( mask_coord,summarize=500000, output_stream = 'file://'+fcm.name )
    tf.print( pred_coord, summarize=500000 , output_stream = 'file://'+fcp.name )
    ### dist
    filename_dist_o = '.o.train.dist.log'
    filename_dist_o = pathname + str(epoch + 1) + filename_dist_o
    fdo = open(filename_dist_o,'a')
    filename_dist_p = '.p.train.dist.log'
    filename_dist_p = pathname + str(epoch + 1) + filename_dist_p
    fdp = open(filename_dist_p,'a')
    tf.print( dist     , summarize=500000 , output_stream = 'file://'+fdo.name )
    tf.print( pred_dist, summarize=500000 , output_stream = 'file://'+fdp.name )
    #######
    fco.close()
    fcm.close()
    fcp.close()
...
def test_step(orign_token, label, edge, coord, dist, len_dictionary, epoch, noise_size_input):
    ...
    (
        ...
        orign_token, coord, dist, label,
        len_dictionary =len_dictionary, noise_size = noise_size_set,noise_size2 = 0.3, #############################!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
import time
#  cal_filename  
filename_test_cal_filename = '../SAVE/1.0_DAMN_liq20/MultiPT/'+ cal_filename + '/test_dataset_acc.log'
filename_train_cal_filename = '../SAVE/1.0_DAMN_liq20/MultiPT/'+ cal_filename + '/train_dataset_acc.log'
filename_test_acc =open( filename_test_cal_filename,'a')
filename_train_acc =open(filename_train_cal_filename,'a')
...
##########     freeze layer   #################################################################
###############################################################################################
model.embed_tokens.trainable=False          #1
model.encoder.trainable=False               #2
model.lm_head.trainable=False               #3
model.gbf_proj.trainable=False              #4
model.gbf.trainable=False                   #5
model.pair2coord_proj.trainable=False       #6
model.dist_head.trainable=False             #7
model.classification_heads.trainable=False  #8 
##############################################################################################
##########    frezze finish  #################################################################
##############################################################################################
##########   test ####
...
for epoch in range(1):
    ...
    #cut_off = random.uniform(6,8)
    cut_off = 7
    tf.print("-----path-----")
    tf.print(path_coord)
#    data_Feeder = Data_Feeder(
#        path_coord, max_batch_system = 1, max_atoms = 130,
#        len_dictionary = len(dictionary), d_cut = cut_off
#        ,Training = True
#    )
#    batch_dataset = data_Feeder.Generate_batch(batch_size=10,Repeat_size=1, shuffle_size=10)
...
#            tf.print("Epoch:", epoch + 1, " batch: ", batch, "lr: ", 
...
#                    output_stream='file://'+filename_train_acc.name)
    ...
    if (epoch + 1) % 1 == 0:
        ...
        test_data_Freeder = Data_Feeder(
                path_test, max_batch_system = 1 , max_atoms = 130,
                len_dictionary = len(dictionary), d_cut = cut_off
                #
                , Training= False
                # learn lable should be False; learn coord shouble be True
        )
        test_data = test_data_Freeder.Generate_test_batch(batch_size=800)
